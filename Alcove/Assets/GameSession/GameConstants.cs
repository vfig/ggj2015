using UnityEngine;
using System.Collections;

public class GameConstants {

	// Gameplay
	public const int PLAYER_COUNT = 2;
	public const int TOWER_SEGMENTS_TO_WIN_GAME = 999;
	public const int NUM_TRIBES_PER_PLAYER = 4;
	public const int TRIBE_STARTING_UNIT_COUNT = 8;

	public const float RECRUITMENT_AREA_DEFAULT_SPAWN_TIME = 0.3f;
	public const float RECRUITMENT_AREA_GROUND_WIDTH = 3.5f;
	public const float RECRUITMENT_AREA_GROUND_Y = -1.2f;
	public const float RECRUITMENT_UNIT_WALK_SPEED = 1.2f;

	public const int PREGAME_DISPLAY_TIME = 90;
	public const int GO_MESSAGE_DISPLAY_TIME = 40;

}

...
public class GameInput {
	public static float GetScrollAxis(int player) {
		if (XCI.GetNumPluggedCtrlrs() >= player) {
			return XCI.GetAxis(XboxAxis.LeftStickY, player);
		} else if (player == 1) {
	...
	public static bool GetTribeButton(int tribe, int player) {
		if (XCI.GetNumPluggedCtrlrs() >= player) {
			return XCI.GetButton(TribeButton(tribe), player);
		} else if (player == 1) {
	...
	public static bool GetTribeButtonDown(int tribe, int player) {
		if (XCI.GetNumPluggedCtrlrs() >= player) {
			return XCI.GetButtonDown(TribeButton(tribe), player);
		} else if (player == 1) {
	...
	public static bool GetTribeButtonUp(int tribe, int player) {
		if (XCI.GetNumPluggedCtrlrs() >= player) {
			return XCI.GetButtonUp(TribeButton(tribe), player);
		} else if (player == 1) {
